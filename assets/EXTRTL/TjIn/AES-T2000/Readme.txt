...
木马程序
木马程序描述
在检测到特定的输入明文序列后，该木马程序通过泄漏电流泄露AES-128的密钥。
泄漏电路（LC）由一个存储密钥的移位寄存器和两个反相器组成。移位寄存器的最低有效位连接到其中一个反相器，该反相器的输出又连接到另一个反相器的输入。
每当最低有效位为“0”时，由第一个反相器的PMOS晶体管和第二个反相器的NMOS晶体管构成的电源与地之间的直接通路会在有限时间内导通。
因此，通过测量泄漏电流即可获取密钥。
