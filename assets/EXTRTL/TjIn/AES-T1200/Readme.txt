...
木马程序
木马程序描述
每当完成128个十六进制值“FFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF”的加密操作后，该木马程序即被激活，并通过隐蔽信道从运行高级加密标准（AES）算法的加密芯片中窃取密钥。
该信道借鉴了扩频通信（也称为码分多址接入，CDMA）的原理，将单个比特的泄露信息分散到多个时钟周期中。
木马程序采用此方法时，会利用伪随机数生成器（PRNG）生成一个CDMA码序列，且该伪随机数生成器以输入明文作为初始化值。
随后，该码序列用于对密钥信息比特进行异或（XOR）调制。调制后的序列被传输至泄漏电路（LC），以便在电源侧信道中建立一个隐蔽的CDMA信道。
LC通过将八个相同的触发器元件连接到异或门的单一输出端来实现，以模拟大电容的效果[1]。
