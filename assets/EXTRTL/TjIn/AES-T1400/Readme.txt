...
木马程序
木马程序描述
一旦检测到预定义的输入明文序列，该木马程序便会针对AES-128分组密码及其对应的密钥编排算法发起攻击。
其核心思路是，在密钥编排过程中人为制造中间状态泄露，这些泄露状态依赖于已知的输入比特和密钥比特，但在密码的正常处理过程中本不会自然出现。
木马程序利用与（AND）逻辑运算，将每个密钥比特与另一个输入比特逐一配对组合。
随后，将所有与门的输出通过异或（XOR）运算合并，得出泄露的中间值。
在密钥编排的每一轮中，该木马程序会泄露一个字节的AES轮密钥。泄露电路（LC）是一个16位移位寄存器，并预先加载了由0和1交替组成的初始序列。
只有当泄露电路的输入为1时，移位寄存器才会被激活，这会导致额外的动态功耗[1]。
