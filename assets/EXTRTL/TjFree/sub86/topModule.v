module top( CLK, RSTN, IA, ID, A, D, Q, WEN,BEN,CE,RD,INT );
...
always@(state,regdest,regsrc,ID,cry,Zregsrc,Sregsrc,sft_out,adder_out,sub_out)
  if (state == `fetch )
  case (ID[15:10])
   6'b000000 : {ncry,alu_out} = 	    adder_out ;  
   6'b000010 : {ncry,alu_out} = {cry,regdest | regsrc};  
   6'b000100 : {ncry,alu_out} = 	    adder_out ;  
   6'b000110 : {ncry,alu_out} = 	      sub_out ;  
   6'b001000 : {ncry,alu_out} = {cry,regdest & regsrc};  
   6'b001010 : {ncry,alu_out} = 	      sub_out ;  
   6'b001100 : {ncry,alu_out} = {cry,regdest ^ regsrc};  
   6'b100010 : {ncry,alu_out} = {cry,	       regsrc};  
   6'b101101 : {ncry,alu_out} = {cry,	      Zregsrc};  
   6'b101111 : {ncry,alu_out} = {cry,	      Sregsrc};  
   default   : {ncry,alu_out} = {cry,regdest	     };  
  endcase
 ...
 begin
   ...
   if ((state == `fetch) || (state ==`shift))
     casex ({ID[15:12],ID[10:9],ID[7]})
      7'b10x0000  : begin RD=0;WR=1; src=ID[5:3]; dest= 3'b111; end  
      7'b100xx10  : begin RD=1;WR=0; src= 3'b111; dest=ID[5:3]; end  
      7'b101xx10  : begin RD=0;WR=0; src= 3'b111; dest=ID[5:3]; end  
      7'b10xxx11  : begin RD=0;WR=0; src=ID[2:0]; dest=ID[5:3]; end  
      7'b00xxx11  : begin RD=0;WR=0; src=ID[2:0]; dest=ID[5:3]; end  
      default   : begin RD=0;WR=0; src=ID[5:3]; dest=ID[2:0]; end  
     endcase
   else if (state==`ret)
...
assign  sft_out = (src   == 3'b111) ? {regdest[31],regdest[31:1]} : 
                  (src   == 3'b101) ? {       1'b0,regdest[31:1]} : 
		                      {regdest[30:0],1'b0       } ; 
